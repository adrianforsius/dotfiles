...
nnoremap ,g :execute "grep -R ". shellescape("<cWORD>") . " ."
nnoremap ,w :%s/\v\s+$//
nnoremap ,W :match Error /\v\s+$/
nnoremap ,a :Ag
nnoremap ,s :mksession
nnoremap ,sv :split $MYVIMRC
vnoremap / /\v
nnoremap / /\v
xmap S <Plug>VSurround
onoremap _ <Plug>(easymotion-tn)
nmap _ <Plug>(easymotion-sn)
vmap _ <Plug>(easymotion-sn)
...
imap  <Plug>Isurround
inoremap \. 
iabbr Appl Application
iabbr Attr Attributes
iabbr @@ adrianforsius@gmail.com
iabbr ccopy Copy right 2014 Adrian Forsius, all rights reserved.
let &cpo=s:cpo_save
unlet s:cpo_save
set autoindent
set autoread
...
set smartindent
set softtabstop=4
set statusline=\ %n\ %*%{&ff}%*%y%*\ %{''.(&fenc!=''?&fenc:&enc).''}\ %<%F%*%m%*%=%5l%*/%L%*%4v\ %**0x%04B\ %*
set tabstop=4
set wildignore=*/tmp/*,*/target/*,*/node_modules/*,*.so,*.swp,*.zip,*\\tmp\\*,*\\target\\*,*.swp,*.zip,*.exe
set wildmenu
set wildmode=longest,list
let s:so_save = &so | let s:siso_save = &siso | set so=0 siso=0
let v:this_session=expand("<sfile>:p")
silent only
cd ~/dotfiles/home/.vim
